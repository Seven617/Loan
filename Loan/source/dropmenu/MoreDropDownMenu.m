...
#define kMarginBetweenImageAndLabel 2
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    return CGSizeMake((kScreenWidth - 2 * KImageMargin - 2 * KImageMargin)/KImageCount*0.8,KImageHeight * FIT_WIDTH*0.7);
}
...
- (void)animateTableView:(UICollectionView *)tableView show:(BOOL)show complete:(void(^)(void))complete {
    ...
    if (show) {
        ...
        CGFloat tableViewHeight = KImageHeight * FIT_WIDTH * ceil(count / KImageCount) + ceil(count / KImageCount) * KImageMargin;
