...
//        menCell.iconImageView.image = image;
        [menCell.iconImageView sd_setImageWithURL:[NSURL URLWithString:self.imagesArray[i]] placeholderImage:[UIImage imageNamed:@"apple"]];
...
- (void)setUpGFScrollMenUI {
    // 创建图片
    self.iconImageView = [[UIImageView alloc] initWithFrame:CGRectMake(ScreenW/8 - 25, 0, 50, 50)];
    self.iconImageView.layer.cornerRadius = 10.0;
    self.iconImageView.clipsToBounds = true;
    self.iconImageView.contentMode = UIViewContentModeScaleAspectFit;  
    [self addSubview:self.iconImageView];
    // 创建标题
    self.titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 60, ScreenW/4, 20)];
    self.titleLabel.font = [UIFont systemFontOfSize:13];
    self.titleLabel.textAlignment = NSTextAlignmentCenter;
    [self addSubview:self.titleLabel];
    // 创建按钮
    self.clickButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, ScreenW, ScreenW)];
